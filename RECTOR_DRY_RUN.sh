#!/usr/bin/env bash
# Run Rector in dry-run mode inside the Docker Compose dev container.
# Usage examples:
#   ./RECTOR_DRY_RUN.sh
#   ./RECTOR_DRY_RUN.sh --config rector.php --memory-limit=512M
...
"${DCMD[@]}" run --rm app composer rector:dry-run "$@"
